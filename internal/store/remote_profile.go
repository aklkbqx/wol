...
func normalizeRemoteProfile(item RemoteProfile) (RemoteProfile, error) {
	...
	case "sunshine":
		defaultPort = 47989
	default:
		return RemoteProfile{}, errors.New("remote protocol must be rdp, vnc, ssh, or sunshine")
	}
	if item.Host == "" {
	...
	if item.Protocol == "sunshine" {
		if item.AppName == "" {
			item.AppName = "Desktop"
		}
		if item.FPS < 0 || item.FPS > 360 {
			return RemoteProfile{}, errors.New("fps must be between 0 and 360")
		}
		if item.BitrateKbps < 0 || item.BitrateKbps > 500000 {
			return RemoteProfile{}, errors.New("bitrate must be between 0 and 500000 kbps")
		}
		if item.Resolution != "" && !resolutionPattern.MatchString(item.Resolution) {
			return RemoteProfile{}, errors.New("resolution must be in format WIDTHxHEIGHT (e.g. 1920x1080)")
		}
	}
...
func (s *Store) GetRemoteProfile(ctx context.Context, deviceID string) (RemoteProfile, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, device_id, protocol, host, port, verify_port, username_hint, domain_hint, certificate_policy, mode, app_name, fps, resolution, bitrate_kbps, enabled, created_at, updated_at FROM remote_profiles WHERE device_id = ?`, strings.TrimSpace(deviceID))
	return scanRemoteProfile(row)
}

func (s *Store) ListRemoteProfiles(ctx context.Context) ([]RemoteProfile, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, device_id, protocol, host, port, verify_port, username_hint, domain_hint, certificate_policy, mode, app_name, fps, resolution, bitrate_kbps, enabled, created_at, updated_at FROM remote_profiles ORDER BY device_id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanRemoteProfile(scanner interface{ Scan(...any) error }) (RemoteProfile, error) {
	...
	var enabled int
	err := scanner.Scan(&item.ID, &item.DeviceID, &item.Protocol, &item.Host, &item.Port, &item.VerifyPort, &item.UsernameHint, &item.DomainHint, &item.CertificatePolicy, &item.Mode, &item.AppName, &item.FPS, &item.Resolution, &item.BitrateKbps, &enabled, &item.CreatedAt, &item.UpdatedAt)
