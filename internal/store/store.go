...
func (s *Store) ListDevices(ctx context.Context) ([]Device, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, mac_address, ip_address, broadcast_address, port, interface_name, site_id, device_type, verify_port, description, enabled, created_at, updated_at FROM devices ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) GetDevice(ctx context.Context, id string) (Device, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, name, mac_address, ip_address, broadcast_address, port, interface_name, site_id, device_type, verify_port, description, enabled, created_at, updated_at FROM devices WHERE id = ?`, id)
...
func scanDevice(scanner interface{ Scan(...any) error }) (Device, error) {
	var item Device
	var enabled int
	err := scanner.Scan(&item.ID, &item.Name, &item.MACAddress, &item.IPAddress, &item.BroadcastAddress, &item.Port, &item.Interface, &item.SiteID, &item.DeviceType, &item.VerifyPort, &item.Description, &enabled, &item.CreatedAt, &item.UpdatedAt)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) CreateDevice(ctx context.Context, item Device) (Device, error) {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO devices (id, name, mac_address, ip_address, broadcast_address, port, interface_name, site_id, device_type, verify_port, description, enabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, item.ID, strings.TrimSpace(item.Name), strings.ToLower(item.MACAddress), item.IPAddress, item.BroadcastAddress, item.Port, item.Interface, item.SiteID, item.DeviceType, item.VerifyPort, item.Description, boolInt(item.Enabled), item.CreatedAt, item.UpdatedAt)
	if err != nil {
...
func (s *Store) UpdateDevice(ctx context.Context, id string, item Device) (Device, error) {
	item.UpdatedAt = time.Now().UTC().Format(time.RFC3339Nano)
	result, err := s.db.ExecContext(ctx, `UPDATE devices SET name = ?, mac_address = ?, ip_address = ?, broadcast_address = ?, port = ?, interface_name = ?, site_id = ?, device_type = ?, verify_port = ?, description = ?, enabled = ?, updated_at = ? WHERE id = ?`, strings.TrimSpace(item.Name), strings.ToLower(item.MACAddress), item.IPAddress, item.BroadcastAddress, item.Port, item.Interface, item.SiteID, item.DeviceType, item.VerifyPort, item.Description, boolInt(item.Enabled), item.UpdatedAt, id)
	if err != nil {
		return Device{}, normalizeDBError(err)
	}
	if count, _ := result.RowsAffected(); count == 0 {
...
func (s *Store) ListGroups(ctx context.Context) ([]Group, error) {
	...
	for rows.Next() {
		...
		item.DeviceIDs, err = s.groupDeviceIDs(ctx, item.ID)
		if err != nil {
			return nil, err
		}
...
func (s *Store) Export(ctx context.Context) (ExportData, error) {
	...
	return ExportData{Version: 1, Sites: sites, Devices: devices, Groups: groups}, nil
}
